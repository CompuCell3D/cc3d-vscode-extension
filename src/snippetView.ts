...
function showOnlyRelevantSnippetsInMenu(): boolean {
    return vscode.workspace.getConfiguration("cc3d").get("showOnlyRelevantSnippetsInMenu") as boolean;
}

export class SnippetView {
    treeDataProvider: TreeDataProvider;

    handleLanguageChange(languageId: string) {
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        const view = vscode.window.createTreeView("snippetView", {
            treeDataProvider: this.treeDataProvider,
            showCollapseAll: true,
        });

        var langId = vscode.window.activeTextEditor?.document.languageId;
        if (langId && showOnlyRelevantSnippetsInMenu()) {
            this.handleLanguageChange(langId);
        }
        ...
        vscode.window.onDidChangeActiveTextEditor((editor) => {
            if (editor && showOnlyRelevantSnippetsInMenu()) {
...
class TreeItem extends vscode.TreeItem {
    ...
    constructor(label: string, children?: TreeItem[]) {
        super(
            label,
            children == null ? vscode.TreeItemCollapsibleState.None : vscode.TreeItemCollapsibleState.Expanded
        );
...
function createTreeFromJson(jsonData: any, rootLabel: string) {
    ...
    for (const [key, value] of Object.entries(jsonData)) {
        var valueAny: any = value; //casting prevents TypeScript compiler error
